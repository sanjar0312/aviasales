  ...
  <Card >
    <Left>
      <Things>
        {things.map(things => (
          <BaggageChoice>
              {openModal && <MyForm openModal={openModal} setOpenModal={setOpenModal} />}
            ...
            {things.price && <BaggagePrice>- {things.price}Сом</BaggagePrice>}
          </BaggageChoice>
        ))}
      </Things>
      {amount && <Warning>Осталось {amount} билета</Warning>}
      <Buy onClick={() => setOpenModal(true)} >
        Купить
        <BuyPrice>за {proposals[0].price}Сом</BuyPrice>
      </Buy>
          ...
          {proposals.slice(1, 3).map(proposals => (
            <Proposal>
              <ProposalsButton>
                ...
                <ProposalPrice>{proposals.price}Сом</ProposalPrice>
              </ProposalsButton>
            </Proposal>
          ))}
          <Proposal>
